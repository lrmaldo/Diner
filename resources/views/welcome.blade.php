...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            header{width:100%;padding:1rem 1.25rem;display:flex;justify-content:flex-end;align-items:center}
            ...
            .logo-wrap{display:flex;align-items:center;justify-content:center;flex-direction:column}
            .logo{width:10rem;height:auto;max-width:90vw;object-fit:contain}
            .subtitle{margin-top:1rem;color:var(--muted)}

            @media (min-width:640px){ .logo{width:14rem} }
            @media (min-width:1024px){ .logo{width:20rem} }

            /* Small fade-in, respects prefers-reduced-motion */
            @media (prefers-reduced-motion: no-preference){
                .logo{opacity:0;transform:translateY(10px);animation:logoIn .7s ease forwards}
