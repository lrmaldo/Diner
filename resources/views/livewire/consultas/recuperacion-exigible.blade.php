            ...
            <table class="min-w-full divide-y divide-gray-300">
                <thead class="bg-blue-600">
                    <tr>
                        ...
                        <th wire:click="sortBy('pendiente')" scope="col" class="px-3 py-3.5 text-right text-sm font-semibold text-white border border-gray-300 border-opacity-70 cursor-pointer hover:bg-blue-700">
                            PENDIENTE
                            @if($sortColumn === 'pendiente')
                                <span>{!! $sortDirection === 'asc' ? '&#8593;' : '&#8595;' !!}</span>
                            @endif
                        </th>
                        <th wire:click="sortBy('eficiencia')" scope="col" class="px-3 py-3.5 text-right text-sm font-semibold text-white border border-gray-300 border-opacity-70 cursor-pointer hover:bg-blue-700">
                            EFEC. %
                ...
                <tfoot class="bg-blue-600 text-white font-bold">
                    <tr>
                        <td class="py-3 pl-4 pr-3 text-left text-sm border border-gray-300 border-opacity-70 uppercase tracking-widest">TOTAL</td>
                        <td class="px-3 py-3 text-center text-sm border border-gray-300 border-opacity-70">{{ number_format($totalExigible, 0) }}</td>
                        <td class="px-3 py-3 text-center text-sm border border-gray-300 border-opacity-70">{{ number_format($totalRecuperado, 0) }}</td>
                        <td class="px-3 py-3 text-center text-sm border border-gray-300 border-opacity-70">{{ number_format($totalPendiente, 0) }}</td>
                        <td class="px-3 py-3 text-center text-sm border border-gray-300 border-opacity-70">{{ number_format($eficienciaTotal, 2) }}%</td>
                    </tr>
                </tfoot>
                @endif
            </table>
