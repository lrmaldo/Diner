<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-project-700 dark:text-project-300">Clientes</h1>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Código postal</th>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                @forelse ($clientes as $cliente)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ trim(implode(' ', array_filter([$cliente->apellido_paterno, $cliente->apellido_materno, $cliente->nombres]))) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $cliente->curp ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $cliente->email ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $cliente->municipio ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $cliente->codigo_postal ?? '-' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ optional($cliente->telefonos->first())->numero ?? '-' }}</td>
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-10 text-center">
                            <div class="max-w-xl mx-auto">
                                ...
                                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">No se encontraron clientes</h3>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Prueba con otros términos de búsqueda o restablece los filtros.</p>
    ...
    @if($confirmingDeleteId)
        <div class="fixed inset-0 z-40 flex items-center justify-center">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg z-50 max-w-md w-full p-6 relative">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Confirmar eliminación</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">¿Estás seguro de que deseas eliminar este cliente? Esta acción no se puede deshacer.</p>
