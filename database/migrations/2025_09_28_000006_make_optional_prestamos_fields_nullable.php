...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        try { DB::statement("ALTER TABLE `prestamos` MODIFY COLUMN `cliente_id` BIGINT UNSIGNED NOT NULL;"); } catch (\Exception $e) {}
        try { DB::statement("ALTER TABLE `prestamos` MODIFY COLUMN `monto_total` DECIMAL(12,2) NOT NULL;"); } catch (\Exception $e) {}
        try { DB::statement("ALTER TABLE `prestamos` MODIFY COLUMN `fecha_primer_pago` DATE NOT NULL;"); } catch (\Exception $e) {}
        try { DB::statement("ALTER TABLE `prestamos` MODIFY COLUMN `periodo_pago` VARCHAR(191) NOT NULL;"); } catch (\Exception $e) {}
        try { DB::statement("ALTER TABLE `prestamos` MODIFY COLUMN `dia_pago` VARCHAR(191) NOT NULL;"); } catch (\Exception $e) {}
        try { DB::statement("ALTER TABLE `prestamos` MODIFY COLUMN `grupo_id` BIGINT UNSIGNED NOT NULL;"); } catch (\Exception $e) {}
    }
};
