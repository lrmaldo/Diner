...
class Create extends Component
{
    ...
    protected function rules(): array
    {
        ...
        $rules = [
            ...
            'fecha_primer_pago' => ['required', 'date', 'after_or_equal:fecha_entrega'],
    ...
    public function enviarAComite(): void
    {
        ...
        if ($this->producto === 'individual') {
            ...
            // Recalcular siempre el monto total desde los miembros en memoria
            $this->normalizeClientesAgregados();
            $total = 0.0;
            foreach ($this->clientesAgregados as $row) {
    ...
    public function addNewClient()
    {
        ...
        $this->validate([
            ...
            'new_ingreso_mensual' => ['required', 'numeric', 'min:0'],
            'new_gasto_mensual_familiar' => ['required', 'numeric', 'min:0'],
            ...
            'new_ingreso_mensual.required' => 'El ingreso mensual es obligatorio.',
            'new_gasto_mensual_familiar.required' => 'El gasto mensual familiar es obligatorio.',
    ...
    public function saveEditedCliente(): void
    {
        ...
        $this->validate([
            ...
            'edit_ingreso_mensual' => ['required', 'numeric', 'min:0'],
            'edit_gasto_mensual_familiar' => ['required', 'numeric', 'min:0'],
            ...
            'edit_ingreso_mensual.required' => 'El ingreso mensual es obligatorio.',
            'edit_gasto_mensual_familiar.required' => 'El gasto mensual familiar es obligatorio.',
        ...
        if ($this->producto === 'individual') {
            ...
            // Actualizar el préstamo físico si ya existe
            if ($this->prestamo_id) {
                $prestamo = Prestamo::find($this->prestamo_id);
                if ($prestamo) {
                    $prestamo->monto_total = $this->monto;
    ...
    public function eliminarMiembro(int $index): void
    {
        ...
        if ($this->prestamo_id && isset($row['cliente_id'])) {
            ...
            if ($prestamo) {
                ...
                // Recalcular y guardar el monto total del préstamo grupal
                $total = 0.0;
                foreach ($this->clientesAgregados as $k => $v) {
                    if ($k !== $index) {
                        $m = $v['monto_solicitado'] ?? 0;
                        if (is_numeric($m) && (float) $m > 0) {
                            $total += (float) $m;
                        }
                    }
